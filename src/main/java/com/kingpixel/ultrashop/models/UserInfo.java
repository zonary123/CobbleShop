...
public class UserInfo {
  ...
  public void read(ServerPlayerEntity player) {
    File folder = Utils.getAbsolutePath(UltraShop.PATH_DATA_USERS);

    if (!folder.exists()) {
      folder.mkdirs();
    }

    CompletableFuture<Boolean> futureRead = Utils.readFileAsync(UltraShop.PATH_DATA_USERS, player.getUuidAsString() + ".json",
      call -> {
        UserInfo userInfo = UltraShop.gson.fromJson(call, UserInfo.class);
        userInfo.check(true);
        DataBaseFactory.users.put(player.getUuid(), userInfo);
      });

    if (!futureRead.join()) {
      String data = UltraShop.gsonWithOutSpaces.toJson(this);
      Utils.writeFileAsync(Utils.getAbsolutePath(UltraShop.PATH_DATA_USERS + player.getUuidAsString() + ".json"), data);
    }
  }
