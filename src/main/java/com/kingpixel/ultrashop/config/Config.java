...
public class Config {
  ...
  public Config readConfig(ShopOptionsApi options) {
    ...
    boolean read = Utils.readFileSync(Utils.getAbsolutePath(options.getPath() + "config.json"), call -> {
      config.set(UltraShop.gson.fromJson(call, Config.class));
  ...
  private static void write(String path, Shop shop) {
    ...
    CompletableFuture<Boolean> futureWrite = Utils.writeFileAsync(finalPath, shop.getId() + ".json", UltraShop.gson.toJson(shop));
    if (!futureWrite.join()) {
      ...
      if (file.isDirectory()) {
        ...
        if (file.getName().endsWith(".json")) {
          ...
          try {
            ...
            shop = UltraShop.gson.fromJson(Utils.readFileSync(file), Shop.class);
            shop.setId(file.getName().replace(".json", ""));
            shop.check();
            shop.setPath(null);
            Utils.writeFileSync(file, UltraShop.gson.toJson(shop));
            shop.setPath(path);
          } catch (IOException e) {
            e.printStackTrace();
          }
  ...
  public void open(ServerPlayerEntity player, ShopOptionsApi options) {
    ...
    if (UIUtils.isInside(this.itemClose.getSlot(), rows)) {
      ItemModel close = UltraShop.lang.getGlobalItemClose(itemClose);
  ...
  private static boolean canOpen(ServerPlayerEntity player, Shop shop) {
    ...
    if (!canopen) {
      PlayerUtils.sendMessage(
        player,
        UltraShop.lang.getMessageShopNotOpen()
