...
public class Lang {
  ...
  public void init(Config config) {
    File folder = Utils.getAbsolutePath(UltraShop.PATH_LANG);
    if (!folder.exists()) {
      folder.mkdirs();
    }
    CompletableFuture<Boolean> futureRead = Utils.readFileAsync(UltraShop.PATH_LANG, config.getLang() + ".json",
      call -> {
        UltraShop.lang = UltraShop.gson.fromJson(call, Lang.class);
        UltraShop.lang.check();
        CompletableFuture<Boolean> futureWrite = Utils.writeFileAsync(UltraShop.PATH_LANG, config.getLang() + ".json",
          UltraShop.gson.toJson(UltraShop.lang));
        if (!futureWrite.join()) {
          CobbleUtils.LOGGER.error("Error writing file: " + UltraShop.PATH_LANG + config.getLang() + ".json");
        }
      });
